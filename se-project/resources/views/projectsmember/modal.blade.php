<div /*popup-sales*/ class="modal fade" id="{{ $role }}-manage" tabindex="-1" role="dialog"
    aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" style="max-width:55%;" role="document">
        <div class="modal-content " style=" background: #92C7CF; ">
            <div class="model-close">
                <button type="button mb-0" class="close d-flex justify-content-end px-2" data-dismiss="modal"
                    aria-label="Close">
            ...
            <div class="modal-body" style=" max-width: 100%;">
                <form id="{{ $role }}Form">
                    <div class="modal-bg p-2"
                        style="max-height:360px; max-width:100%;background: #ffffff; border-radius: 20px; overflow:auto;">

                        <div class="gg" style=" border-radius: 20px;background: #FBF9F1;">
                            <span class="mb-0 p-2 px-4"
                                style=" max-width: 100%; font-size: 24px;white-space: pre; margin: 0;"> รหัสพนักงาน
                                ชื่อ-สกุล </span>
                        </div>
                        ...
                        <div id="checkboxes">
                            ...
                            @foreach ($depart_em as $item)
                                @if ($item->role == $role)
                                    <div class="detail d-flex justify-content-start border-bottom px-3 align-items-center"
                                        style="display: flex; align-items: center;">
                                        <input type="checkbox" name="selectedEmployees[]" value="{{ $item->user_id }}"
                                            id="checkbox{{ $item->user_id }}"
                                            data-department-name="{{ $item->role }}">
                                        <label for="checkbox{{ $item->user_id }}" class="mb-0 px-2"
                                            style="display: flex; align-items: center;"></label>
                                        <span
                                            style="display: flex; align-items: center; font-size: 24px;white-space: pre; margin: 0;">{{ $item->employee_id }}
                                            {{ $item->username }}</span>
                                    </div>
                                @endif
                            @endforeach
        ...
        $('#{{ $role }}-manage').on('shown.bs.modal', function(e) {
            @foreach ($project_members as $pm)
                var checkbox = document.querySelector(
                    'input[name="selectedEmployees[]"][value="{{ $pm->user_id }}"]');
                if (checkbox) {
